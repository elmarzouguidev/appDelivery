            ...
            <tr class="information">
                <td colspan="6">
                    <table>
                        <tr>
                            <td style="width: 50% ;">
                                @if(optional($invoice->delivery)->type == 'entreprise' && optional($invoice->delivery)->company_name)
                                <strong> Société : {{ optional($invoice->delivery)->company_name }}</strong> <br />
                                @else
                                <strong> Livreur : {{ optional($invoice->delivery)->full_name }}</strong> <br />
                                @endif

                                @if(optional($invoice->delivery)->type == 'particulier')
                                 CNIE : {{ strtoupper(optional($invoice->delivery)->cnie) }}<br />
                                @endif

                                @if(optional($invoice->delivery)->type == 'entreprise')
                                 ICE : {{ optional($invoice->delivery)->company_ice }}<br />
                                @endif

                                 Adresse : {{ optional($invoice->delivery)->addresse }} <br />
